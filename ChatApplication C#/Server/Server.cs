    ...
    public partial class Server : Form
    {
        ...
        { // needs error handling TODO
          // Users class is for adding users to list of users
            ...
            User client = new User(_clientSocket);
        ...
        private void RecieveCallBack(IAsyncResult AR)
        {
            try
            {
                ...
                if (currentUser.RsaPrivateServerKey != null && currentUser.AesPrivateKey != null)
                {
                    ...
                    // Deserilize data
                    DataSerializerServer DD = new DataSerializerServer(databuf);
                    if (DD.AccountSetBool)
                    {
                        ...
                        if (!AM.AccountNameExists(DD.Name))
                        {
                            ...
                            DataSerializerServer DSS = new DataSerializerServer(DD.Name, "", "True");
                        ...
                        else
                        {
                            ...
                            DataSerializerServer DSS = new DataSerializerServer(DD.Name, "", "False");
                            byte[] AccountPacket = DSS.AccountPacketToByteArray();
                            byte[] EncryptedAccountPacket = EncryptWithAES(AccountPacket, currentUser);
                            _clientSocket.BeginSend(EncryptedAccountPacket, 0, EncryptedAccountPacket.Length, SocketFlags.None, new AsyncCallback(SendCallBack), _clientSocket);
                        }
                    ...
                    else if (DD.LoginGetBool)
                    {
                        ...
                        if (AM.CheckLogin(DD.Name, DD.Password))
                        {
                            ...
                            if (users.UserLoggedIn(DD.Name))
                            {
                                ...
                                DataSerializerServer DSS = new DataSerializerServer(DD.Name, "", "Login");
        ...
        private void SendToAllClients(string name, string message)
        {
            ...
            //Users users = new();
            DataSerializerServer DD = new DataSerializerServer(name, message);
        ...
        {
            DataSerializerServer DD = new DataSerializerServer(aes.Key, aes.IV);
        ...
        private void richTextChat_TextChanged(object sender, EventArgs e)
        {
        ...
        private void richTextServer_TextChanged(object sender, EventArgs e)
        {

            // set the current caret position to the end
            richTextServer.SelectionStart = richTextServer.Text.Length;
            // scroll it automatically
            richTextServer.ScrollToCaret();
        }
    }
}
